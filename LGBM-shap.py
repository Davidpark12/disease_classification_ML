...
#train = pd.read_csv("dataset_park_median_MODY(25).csv", encoding="CP949" )
...
lgb_cf = LGBMClassifier(colsample_bytree=0.85,subsample=0.9, num_leaves=2, min_child_samples=30, n_estimators=500,
                        learning_rate=0.1)
...
import shap
cmap = plt.cm.get_cmap("prism", 50)
explainer = shap.TreeExplainer(lgb_cf).shap_values(X_train2)
shap.summary_plot(explainer, X_train2, color=cmap, class_names =pd.Series(y.unique()).sort_values().values)
...
#
# importance_df = pd.DataFrame((lgb_cf.feature_importances_/pd.Series(lgb_cf.feature_importances_).values.sum())*100)
